...
static int sendMessagePingFrame(struct lws *wsi) {

	unsigned char pingbuf[LWS_PRE + 1];
	int n = lws_write(wsi, pingbuf + LWS_PRE, 1, LWS_WRITE_PONG);
	return n;
}
...
static int ws_service_callback(struct lws *wsi, enum lws_callback_reasons reason, void *user, void *in, size_t len) {
	...
	case LWS_CALLBACK_CLIENT_ESTABLISHED: {
		...
		lwsl_notice("Connect[%s] connected server success.\n", tool->name.c_str());
		tool->connection_flag = 1;
	} break;
	case LWS_CALLBACK_CLIENT_CONNECTION_ERROR: {
		struct pthread_args *tool = (struct pthread_args *)__g_pthread_args[wsi];
		lwsl_notice("Connect[%s] with server error.\n", tool->name.c_str());
		tool->destroy_flag = 1;
		tool->connection_flag = 0;
	} break;
	case LWS_CALLBACK_CLOSED: {	
		struct pthread_args *tool = (struct pthread_args *)__g_pthread_args[wsi];
		lwsl_notice("Connect[%s] LWS_CALLBACK_CLOSED\n", tool->name.c_str());
		tool->destroy_flag = 1;
		tool->connection_flag = 0;
	} break;
	case LWS_CALLBACK_CLIENT_RECEIVE: {
		...
		buf[len] = '\0';
		lwsl_notice("Client[%s] recvived:%s\n", tool->name.c_str(), buf);
		free(buf);
	} break;
...
static void *pthread_routine(void *tool_in)
{
	...
	static std::string s = "abcdefghijklmnopqrstuvwxyz0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ~!@#$%^&*()_+`=,./<>?";
	while (!tool->destroy_flag)
	{
		static int i = 0;
		char buf[512];
		sprintf(buf, "{\"client\":%s, \"times\":%d, \"extra\":\"%s\"}", tool->name.c_str(), i++, s.c_str());
...
static void create_ws_client(int v)
{
	...
	lwsl_debug("[Main] context created.\n");
	...
	lwsl_debug("[Main] wsi create success.\n");
