...
//	MeterFilter verifier(new FileSink("c:\\dt.tmp"));
	...
	{
#ifdef CRYPTOPP_WIN32_AVAILABLE
		OutputDebugString(TEXT("Crypto++ DLL integrity check failed. Cannot open file for reading."));
#endif
		return false;
	}
	...
	{
		OutputDebugString(TEXT("In memory integrity check failed. This may be caused by debug breakpoints or DLL relocation.\n"));
		moduleStream.clear();
